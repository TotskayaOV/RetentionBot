...
class ShowAirC(StatesGroup):
    general_date = State()


@dp.message_handler(commands=['show_airc'], state=None)
...
@dp.message_handler(state=ShowAirC.general_date, content_types=ContentTypes.ANY)
async def general_date_record(message: Message, state: FSMContext):
    # try:
        ...
        text = show_date_data_db(air_db.get_airtable_comment, date_update)
