...
from controller import writing_airtable
from view.Keyboards import kb_cancel_fsm, create_kb_name_files, filenames_data



class NewAir(StatesGroup):
    general_date = State()
    upload_file = State()



@dp.callback_query_handler(filenames_data.filter(files_name='airF'), state=None)
...
@dp.message_handler(state=NewAir.general_date, content_types=ContentTypes.ANY)
...
async def work_record(message: Message, state: FSMContext):
    if document := message.document:
        ...
        try:
            os.rename(f'./cred/{document.file_name}', './cred/airtable.csv')
        except:
            os.remove('./cred/airtable.csv')
            os.rename(f'./cred/{document.file_name}', './cred/airtable.csv')
        finally:
            try:
                writing_airtable(date_update)
                ...
                await message.answer(text='Данные Airtable загружены', reply_markup=create_kb_name_files(date_update))
            finally:
                try:
                    os.remove('./cred/airtable.csv')
                    await state.reset_data()
