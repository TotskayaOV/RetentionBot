...
class ShowCall(StatesGroup):
    general_date = State()


@dp.message_handler(commands=['show_call'], state=None)
...
@dp.message_handler(state=ShowCall.general_date, content_types=ContentTypes.ANY)
async def general_date_record(message: Message, state: FSMContext):
    # try:
        ...
        text = show_date_data_db(call_db.get_call, date_update)
